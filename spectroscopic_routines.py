...
def image_header(image,keyword):
    ...
    image = fits.getdata(image)#,ext=1)    
...
def twodsky(image,object_keyword):
    ...
    # blockwid = int(2.5*len(ys[rowaverage > 0.1]))
    # peakdex = np.argmax(rowaverage)
    # block_x = ys[peakdex-blockwid:peakdex+blockwid]
    # block_y = rowaverage[peakdex-blockwid:peakdex+blockwid]
    #
    # plt.plot(ys, rowaverage)
    # plt.plot(block_x, block_y)
    ...
    ngrow = int(1.5*len(objrows)) # number of rows to include above and below object rows
    newobjrows = []
    for row in objrows:
    ...
    def get_profile_spl(dls, stamp):
        ...
        t = np.linspace(x.min() + 1, x.max() - 1, int(x.max() - x.min()))
    ...
    plt.close()
    # fits.writeto(filename='sky_'+image_name,data=np.array(sky),header=header,overwrite=True)        #APM:output sky background
...
def optimal(image_raw, sky, xs, ys, nx, ny, yvals,trace_c,display):
    ...
    #TED ADDITION! - set counts to 0 in the gap regions to avoid weirdness
    normed[:,1021:1073] = 0
    normed[:,2097:2145] = 0
    #END ADDITION
...
def wavelength(onedspec_sum,onedspec_optimal,spec_file_name,wave_min,wave_max,arc_file,reference_spec,line_list,trace,trace_fwhm,
    ...
    ### APM addition: for resolution of arc lamps
    # cols = np.arange(lamp_spec.size)
    # sigma_guess = 2.5
    # gaus_sig_maybe = []; wav_maybe = []
    # for line in lamp_lines:
    #     if line['counts'] > 60000:
    #         # line is saturated...skip
    #         continue
    ...
    #     guess = (line['counts'], line['col'], sigma_guess)
    #     bounds = ((0, line['col'] - 3, 0), (np.inf, line['col'] + 3, np.inf))
    #     # print(line['wav'],line['y'])
    #     # print(cols[i0:i1])
    #     # print(lamp_spec[i0:i1])
    #     try:
    #         popt, pcov = curve_fit(gaussian, cols[i0:i1], lamp_spec[i0:i1], p0=guess, bounds=bounds)
    #     except RuntimeError:
    #         # curve_fit failed to converge...skip
    #         continue
    #     # print(popt)
    #     if popt[2] <= 2.75 and popt[2] >= 2.25 and line['used']:
    #         gaus_sig_maybe.append(line['wav']/popt[2]*gaussian_sigma_to_fwhm)
    #         wav_maybe.append(line['wav'])
    #         print(line['wav'],np.round(line['wav']/popt[2]*gaussian_sigma_to_fwhm,3))
    # print(np.mean(gaus_sig_maybe))
    #     # print(line['wav']/popt[2]*gaussian_sigma_to_fwhm)
    # plt.plot(wav_maybe,gaus_sig_maybe,'.k')
    # plt.title('32')
    ...
    return interact,wavs_more               #APM: return wavs_more -- calibrated wavelength
...
def flux_callibration(standard_reduced_spec,standard_name,science_spec,display):
    ...
    def response(w,f):
        ...
        t = np.arange(calspec['wav'][1], calspec['wav'][-2], int(np.median(calspec['dlam'])))
        # print (calspec['dlam'])
